...
echo "$($PSQL "SELECT c.course FROM students AS s INNER JOIN majors as m USING(major_id) INNER JOIN majors_courses AS mc USING(major_id) INNER JOIN courses AS c USING(course_id) GROUP BY mc.course_id, c.course_id HAVING COUNT(course_id) = 1 ORDER BY course;")"
